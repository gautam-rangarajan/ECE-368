...
int NodeLR(treeNode * parent, treeNode * child) // returns 1 for left and 0 for right
{
...
void reroot(treeNode * root, treeNode * parent, treeNode * curNode, double * minH, double * minW, double * minA)
{
	...
	if(parent == NULL || root == NULL || curNode == NULL)
	{
		return;
	}
	...
	//LEFT RE-ROOT (retain left child of curNode)
	//left reroot
	temp = curNode->right;
	curNode->right = root;
	if(LR == 1)
	...
	//RIGHT RE-ROOT (retain right child of curNode)
	//right reroot
	temp = curNode->left;
	curNode->left = root;
	if(LR == 1)
